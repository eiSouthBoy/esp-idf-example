...
static void esp_gap_cb(esp_gap_ble_cb_event_t event, esp_ble_gap_cb_param_t *param)
{
    ...
    case ESP_GAP_BLE_SCAN_RESULT_EVT:
    {
        ...
        case ESP_GAP_SEARCH_INQ_RES_EVT: // 获取扫描结果
            ...
            // 解析报文
            if (31 == scan_result->scan_rst.adv_data_len)
            {
                char mac[18] = {0};
                for (int i = 0; i < 6; i++)
                {
                    char chunk[4] = {0};
                    if (i == 5)
                        sprintf(chunk, "%02x", scan_result->scan_rst.bda[i]);
                    else
                        sprintf(chunk, "%02x:", scan_result->scan_rst.bda[i]);
                    strcat(mac, chunk);
                }
                mac[18 - 1] = '\0';
                printf("%s\n", mac);
                if (!strcmp(mac, "66:55:44:33:22:11"))
                {
                    char data[128] = {0};
                    for (int i = 0; i < scan_result->scan_rst.adv_data_len; i++)
                    {
                        char chunk[4] = {0};
                        sprintf(chunk, "%02x ", scan_result->scan_rst.ble_adv[i]);
                        strcat(data, chunk);
                    }
                    data[128 - 1] = '\0';

                    ESP_LOGI(GATTC_TAG, "----> len=31, mac: %s, data: %s", mac, data);
                }
            }
...
void app_main(void)
{
    ...
    // 注册 GATT APP 回调函数，负责管理 client 连接到 server 之后发生的事件。
    // 例如 搜索服务和读写数据
