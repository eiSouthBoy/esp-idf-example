...
#define FLOW_CONTROL_QUEUE_LENGTH (1024)
...
static esp_err_t pkt_eth2wifi(esp_eth_handle_t eth_handle, uint8_t *buffer, 
                    uint32_t len, void *priv)
{
    ...
    if (xQueueSend(flow_control_queue, &msg, 
            pdMS_TO_TICKS(FLOW_CONTROL_QUEUE_TIMEOUT_MS)) != pdTRUE) 
    {
...
static void eth2wifi_flow_control_task(void *args)
{
    ...
    while (1) 
    {
        if (xQueueReceive(flow_control_queue, &msg,
                          pdMS_TO_TICKS(FLOW_CONTROL_QUEUE_TIMEOUT_MS)) == pdTRUE)
        {
            timeout = 0;
...
{
    ESP_LOGI(TAG, "FLOW_CONTROL_QUEUE_LENGTH: %d", FLOW_CONTROL_QUEUE_LENGTH);
    ...
    BaseType_t ret = xTaskCreate(eth2wifi_flow_control_task, 
                                 "flow_ctl", 
                                 2048, 
                                 NULL, 
                                 (tskIDLE_PRIORITY + 2), 
