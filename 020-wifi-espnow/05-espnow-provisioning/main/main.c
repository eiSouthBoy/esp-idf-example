                    ...
                    size_t size, wifi_pkt_rx_ctrl_t *rx_ctrl)
{
     ...
     */
    ESP_LOGI(TAG, "MAC: "MACSTR", Channel: %d, RSSI: %d, Product_id: %s, "
             "Device Name: %s, Auth Mode: %d, device_secret: %s",
...
static esp_err_t app_responder_init()
{
    ...
    // 响应者开始发送 provision beacon，持续 30 秒
    ret = espnow_prov_responder_start(&responder_info, pdMS_TO_TICKS(30 * 1000), 
                                      &wifi_config, app_espnow_prov_recv_cb);
        ...
        size_t size, wifi_pkt_rx_ctrl_t *rx_ctrl)
{
    ...
    ESP_LOGI(TAG, "responder info || MAC: "MACSTR", Channel: %d, RSSI: %d, wifi_mode: %d, "
             "ssid: %s, password: %s, token: %s",
             MAC2STR(src_addr), rx_ctrl->channel, rx_ctrl->rssi, wifi_config->mode, 
             wifi_config->sta.ssid, wifi_config->sta.password, wifi_config->token);
...
static esp_err_t app_initiator_init()
{
    ...
    for (;;) 
    {
        // 配网信标扫描初始化，阻塞并等待portMAX_DELAY时间
        ret = espnow_prov_initiator_scan(responder_addr, &responder_info, 
                                         &rx_ctrl, portMAX_DELAY);
        ESP_ERROR_CONTINUE(ret != ESP_OK, "espnow_prov_responder_beacon");

        ESP_LOGI(TAG, "responder info || MAC: "MACSTR", Channel: %d, RSSI: %d, "
                 "Product_id: %s, Device Name: %s",
        ...
        ret = espnow_prov_initiator_send(responder_addr, &initiator_info, 
                                         app_espnow_prov_recv_cb, pdMS_TO_TICKS(3 * 1000));
        ESP_ERROR_CONTINUE(ret != ESP_OK, "<%s> espnow_prov_initiator_send", 
                           esp_err_to_name(ret));
...
#ifdef CONFIG_APP_ESPNOW_PROV_INITIATOR
    // initiator 初始化
    ESP_LOGI(TAG, "initiator mode");
    app_initiator_init();
#elif CONFIG_APP_ESPNOW_PROV_RESPONDER
    // responder 初始化
    ESP_LOGI(TAG, "responeder mode");
    app_responder_init();
#endif
}
