                                ...
                                int32_t event_id, void* event_data)
{
    ...
    case WIFI_EVENT_STA_DISCONNECTED:
        ...
        {
            // WIFI STA重连最大次数后，仍然失败，通知BLE Client
            gl_sta_is_connecting = false;
...
static void initialise_wifi(void)
{
    ...
    // 分别创建默认的 sta 和 ap 接口实例
    esp_netif_t *sta_netif = esp_netif_create_default_wifi_sta();
    assert(sta_netif);
    esp_netif_t *ap_netif = esp_netif_create_default_wifi_ap();
    assert(ap_netif);

    // 注册 WIFI_EVENT 和 IP_EVENT 事件回调函数
...
static esp_blufi_callbacks_t example_callbacks = {
    .event_cb = example_event_callback, // 处理蓝牙事件回调函数
    .negotiate_data_handler = blufi_dh_negotiate_data_handler, // DH算法进行秘钥协商
    .encrypt_func = blufi_aes_encrypt, // AES加密
    .decrypt_func = blufi_aes_decrypt,  // AES解密
    .checksum_func = blufi_crc_checksum,    // CRC校验
};
...
static void example_event_callback(esp_blufi_cb_event_t event, esp_blufi_cb_param_t *param)
{
    ...
    switch (event) {
    case ESP_BLUFI_EVENT_INIT_FINISH:   // 初始化
        BLUFI_INFO("BLUFI init finish\n");

        // 开始发送BLE广播数据
        esp_blufi_adv_start();
        break;
    case ESP_BLUFI_EVENT_DEINIT_FINISH: // 反初始化
        BLUFI_INFO("BLUFI deinit finish\n");
        break;
    case ESP_BLUFI_EVENT_BLE_CONNECT:   // 有客户端连接
        BLUFI_INFO("Recv BLUFI ble Client connect request\n");
        ble_is_connected = true;

        // 停止发送BLE广播数据
        ESP_LOGI(TAG, "BLUFI ble adv stop");
        esp_blufi_adv_stop();

        // 加密实例初始化
        ESP_LOGI(TAG, "BLUFI security init");
        blufi_security_init();
        break;
    case ESP_BLUFI_EVENT_BLE_DISCONNECT:    // 断开
        ...
        esp_blufi_adv_start();
        break;
    case ESP_BLUFI_EVENT_SET_WIFI_OPMODE:   // 接口客户端配网信息
        BLUFI_INFO("Recv BLUFI Set WIFI opmode %d\n", param->wifi_mode.op_mode);

        // 设置 WIFI 工作模式
        ESP_LOGI(TAG, "--> esp_wifi_set_mode()");
        ESP_ERROR_CHECK(esp_wifi_set_mode(param->wifi_mode.op_mode));
        break;
    case ESP_BLUFI_EVENT_REQ_CONNECT_TO_AP: // 连接AP请求
        BLUFI_INFO("Recv BLUFI requset wifi connect to AP\n");
        /* there is no wifi callback when the device has already connected to this wifi
        so disconnect wifi before connection.
        */
        // 连接目标WIFI前，先断开当前的WIFI
        ...
        example_wifi_connect();
        break;
    case ESP_BLUFI_EVENT_REQ_DISCONNECT_FROM_AP: // 断开AP请求
        BLUFI_INFO("BLUFI requset wifi disconnect from AP\n");

        // 断开当前的WIFI
        esp_wifi_disconnect();
        break;
    case ESP_BLUFI_EVENT_REPORT_ERROR:  // 错误
        BLUFI_ERROR("BLUFI report error, error code %d\n", param->report_error.state);

        // 发送错误消息
        esp_blufi_send_error_info(param->report_error.state);
        break;
    case ESP_BLUFI_EVENT_GET_WIFI_STATUS:  // BLE Client请求获取WIFI连接状态
    {
        BLUFI_INFO("Recv get wifi status request\n");
        wifi_mode_t mode;
        esp_blufi_extra_info_t info;
        uint8_t soft_conn_num;

        // 获取当前 WIFI 工作模式
        esp_wifi_get_mode(&mode);

        soft_conn_num = softap_get_current_connection_number();
        if (gl_sta_connected) // STA已连接到目标AP
        {
            memset(&info, 0, sizeof(esp_blufi_extra_info_t));
            memcpy(info.sta_bssid, gl_sta_bssid, 6);
            info.sta_bssid_set = true;
            ...
            // 发送WIFI STA连接状态（是否获取IP，连接的BSSID、SSID），以及WIFI AP有多少了STA连接
            esp_blufi_send_wifi_conn_report(mode, gl_sta_got_ip, 
                    gl_sta_got_ip ? ESP_BLUFI_STA_CONN_SUCCESS : ESP_BLUFI_STA_NO_IP, &info);
        } 
        else if (gl_sta_is_connecting) // STA正在连接目标AP
        {
            esp_blufi_send_wifi_conn_report(mode, ESP_BLUFI_STA_CONNECTING, 
                    soft_conn_num, &gl_sta_conn_info);
        } 
        else 
        {
            esp_blufi_send_wifi_conn_report(mode, ESP_BLUFI_STA_CONN_FAIL, 
    ...
    case ESP_BLUFI_EVENT_RECV_SLAVE_DISCONNECT_BLE: // 关闭GATT会话
        BLUFI_INFO("blufi close a gatt connection");

        // 主动断开连接会话
        esp_blufi_disconnect();
        break;
    case ESP_BLUFI_EVENT_DEAUTHENTICATE_STA:    // 取消STA认证
        /* TODO */
        break;
	case ESP_BLUFI_EVENT_RECV_STA_BSSID:    // 接收STA的BSSID
        ...
        break;
	case ESP_BLUFI_EVENT_RECV_STA_SSID: // 接收STA的SSID
        ...
        break;
	case ESP_BLUFI_EVENT_RECV_STA_PASSWD: // 接收STA的密码
        ...
        break;
	case ESP_BLUFI_EVENT_RECV_SOFTAP_SSID:  // 接口SOFTAP的SSID
        ...
        break;
	case ESP_BLUFI_EVENT_RECV_SOFTAP_PASSWD:    // 接收SOFTAP的密码
        ...
        BLUFI_INFO("Recv SOFTAP PASSWORD %s len = %d\n", ap_config.ap.password, 
                    param->softap_passwd.passwd_len);
        break;
	case ESP_BLUFI_EVENT_RECV_SOFTAP_MAX_CONN_NUM:  // 接收SOFTAP最大连接数量
        ...
        break;
	case ESP_BLUFI_EVENT_RECV_SOFTAP_AUTH_MODE: // 接收SOFTAP加密模式
        ...
        break;
	case ESP_BLUFI_EVENT_RECV_SOFTAP_CHANNEL:   // 接受SOFTAP的信道
        ...
        break;
    case ESP_BLUFI_EVENT_GET_WIFI_LIST: // 获取WIFI列表
    {
    ...
    case ESP_BLUFI_EVENT_RECV_CUSTOM_DATA: // 接受用户自定义数据
...
// 编译菜单中，是否开启蓝牙控制器，是否关闭 NIMBLE。当前编译菜单设置满足条件
#if CONFIG_BT_CONTROLLER_ENABLED || !CONFIG_BT_NIMBLE_ENABLED
    // 蓝牙控制器初始化
    ESP_LOGI(TAG, "--> esp_blufi_controller_init()");
    ...
    // 蓝牙协议栈初始化，接着注册用户自定义回调函数
    ESP_LOGI(TAG, "--> esp_blufi_host_and_cb_init()");
