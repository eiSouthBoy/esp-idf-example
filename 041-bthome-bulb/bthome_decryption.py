...
def parse_button_dimmer_value(data: bytes):
    """Parse decrypted payload to readable BTHome data"""
    vlength = len(data)
...
def main() -> None:
    ...
    data = bytes(bytearray.fromhex("3A013C0000"))
    ...
    mac = binascii.unhexlify("9015067C1B66")  # MAC
    uuid16 = b"\xd2\xfc"
    sw_version = b"\x41"
    ...
    print()
    print("====== Test decode -----------------------------------------")
    payload2 = "02010604094449591116d2fc41b2105b00702302000086d406f8"
    print("encrytion data: ", payload2)
    decrypt_aes_ccm(key=bindkey, mac=mac, data=payload)
