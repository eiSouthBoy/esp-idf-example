...
// 0x231d39c1d7cc1ab1aee224cd096db932
...
static const uint8_t peer_mac[] = {0x90, 0x15, 0x06, 0x7c, 0x1b, 0x66};

static QueueHandle_t s_scan_result_queue;

static led_strip_handle_t configure_led(void)
{
    ESP_LOGI(TAG, "strip_gpio_num: %d, max_leds: %d, resolution_hz: %d", 
...
ble_hci_addr_t s_bda = {0x90, 0x15, 0x06, 0x7c, 0x1b, 66}; 
static void ble_hci_scan_cb(ble_hci_scan_result_t *scan_result, uint16_t result_len)
{
    if (scan_result->bda[0] == peer_mac[0] &&
        scan_result->bda[1] == peer_mac[1] &&
        scan_result->bda[2] == peer_mac[2] &&
        scan_result->bda[3] == peer_mac[3] &&
        scan_result->bda[4] == peer_mac[4] &&
        scan_result->bda[5] == peer_mac[5])
    {
        char addr[64] = {0};
        char *padr = addr;
        for (uint8_t i = 0; i < 6; i++)
        {
            sprintf(padr, "%02x ", scan_result->bda[i]);
            padr += 3;
        }
        ESP_LOGI(TAG, "evt: %d, type: %d", 
                scan_result->search_evt, 
                scan_result->dev_type);
        for (int i = 0; i < scan_result->adv_data_len; i++)
            printf("%02x ", scan_result->ble_adv[i]);
        printf("\n");
...
static void configure_ble_scan(void)
{
    ...
    ret = ble_hci_set_scan_enable(true, true);
    if (ESP_OK != ret)
    {
        ESP_LOGW(TAG, "ble_hci_set_scan_enable() failed");
    }
}
            ...
            for (int i = 0; i < reports->num_reports; i++) 
            {
                ...
                if (reports->report[i].id == BTHOME_EVENT_ID_BUTTON) 
                {
                    if (reports->report[i].data[0] == 0x01) 
                    {
                        ...
                        ESP_LOGI(TAG, "A, led status: %d", led_onoff);
                        led_onoff = !led_onoff;
                        ESP_LOGI(TAG, "B, led status: %d", led_onoff);
                    }
                }

                /* 2. handle dimmer event */
